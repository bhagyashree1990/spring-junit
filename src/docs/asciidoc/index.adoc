= Spring REST Docs JUnit 5 Sample
Bhagyashree Chavan - {version}, {date} | {hostname} - {ip} 
:doctype: book
:icons: font
:source-highlighter: highlightjs
:highlightjs-theme: github
:toc: left
:toclevels: 3
:sectlinks:
:sectnums:

== Greetings
Sample REST API demonstration of Spring REST Docs with JUnit 5.

To make a request using cURL:

include::{snippets}/greeting/curl-request.adoc[]

HTTP request:

include::{snippets}/greeting/http-request.adoc[]

HTTP request params:

include::{snippets}/greeting/request-parameters.adoc[]

HTTP response:

include::{snippets}/greeting/http-response.adoc[]

== Persons

=== Fetch the youngest person information.

To make a request using cURL:

include::{snippets}/person/curl-request.adoc[]

HTTP request:

include::{snippets}/person/http-request.adoc[]

HTTP response:

include::{snippets}/person/http-response.adoc[]

HTTP response fields:

include::{snippets}/person/response-fields.adoc[]

=== Fetch all persons information.

To make a request using cURL:

include::{snippets}/person-all/curl-request.adoc[]

HTTP request:

include::{snippets}/person-all/http-request.adoc[]

HTTP response:

include::{snippets}/person-all/http-response.adoc[]

HTTP response fields:

include::{snippets}/person-all/response-fields.adoc[]
